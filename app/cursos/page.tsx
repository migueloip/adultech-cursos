        ...
        <div className="container mx-auto px-4 py-4 max-w-full overflow-hidden">
          <nav className="flex flex-col sm:flex-row items-center justify-between gap-4 min-w-0">
            <div className="flex items-center space-x-3 min-w-0 flex-shrink">
              ...
              />
              <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-slate-800 dark:text-white truncate">AdulTech Cursos</h1>
            </div>
            <Button
              asChild
              variant="outline"
      ...
      <main className="py-12">
        <div className="container mx-auto px-4 max-w-full overflow-hidden">
          <div className="text-center mb-12 px-2">
            <h2 className="text-4xl md:text-5xl font-bold text-slate-800 dark:text-white mb-6 break-words">Nuestros Cursos</h2>
            <p className="text-xl md:text-2xl text-slate-600 dark:text-slate-300 max-w-3xl mx-auto leading-relaxed break-words">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 max-w-7xl mx-auto px-2">
            {cursos.map((curso) => {
              const IconComponent = iconMap[curso.icono as keyof typeof iconMap] || Smartphone
              return (
                <Card
                  key={curso.id}
                  className={`${curso.color} dark:bg-slate-800 border-2 hover:shadow-lg transition-shadow w-full min-w-0 overflow-hidden`}
                >
                  <CardHeader className="text-center pb-4 px-4">
                    <div className="mx-auto mb-4 p-4 rounded-full bg-white/80 dark:bg-slate-700/80 flex-shrink-0">
                      <IconComponent className={`h-12 w-12 ${curso.icon_color}`} />
                    </div>
                    <CardTitle className="text-xl md:text-2xl font-bold text-slate-800 dark:text-white leading-tight break-words hyphens-auto">
                      {curso.titulo}
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="text-center px-4">
                    <p className="text-lg text-slate-600 dark:text-slate-300 mb-6 leading-relaxed break-words">
                      {curso.descripcion}
                    </p>
                    <Button asChild size="lg" className="w-full text-lg py-3 min-w-0">
                      <Link href={`/cursos/${curso.id}`} className="flex items-center justify-center">
                        <Play className="mr-2 h-5 w-5 flex-shrink-0" />
                        <span className="truncate">Comenzar Curso</span>
                      </Link>
                    </Button>
          ...
          <div className="mt-16 text-center px-2">
            <Card className="max-w-2xl mx-auto bg-white/80 dark:bg-slate-800/80 border-2 border-blue-200 dark:border-blue-700 w-full min-w-0 overflow-hidden">
              <CardContent className="p-4 sm:p-6 lg:p-8">
                <h3 className="text-2xl md:text-3xl font-bold text-slate-800 dark:text-white mb-4 break-words">
                  ¿Necesitas ayuda?
                </h3>
                <p className="text-lg text-slate-600 dark:text-slate-300 mb-6 leading-relaxed break-words">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <Button
                    ...
                    size="lg"
                    variant="outline"
                    className="text-lg bg-white dark:bg-slate-700 border-2 border-slate-300 dark:border-slate-600 text-slate-800 dark:text-white hover:bg-slate-100 dark:hover:bg-slate-600 min-w-0"
                  >
                    <Link href="/preguntas" className="truncate">Ver Preguntas Frecuentes</Link>
                  </Button>
                  <Button asChild size="lg" className="text-lg min-w-0">
                    <Link href="/contacto" className="truncate">Contactar Soporte</Link>
                  </Button>
                </div>
              </CardContent>
            </Card>
